...
import "@/styles/nav-orbit-mobile.css";

/* Dots on TOP arc of Ring 7 (r=153.8), center=(332.26, 332.44) */
const NAV_ITEMS = [
  { label: "About us",       cx: 199.1, cy: 255.5 },
  { label: "Verticals",      cx: 241.9, cy: 208.0 },
  { label: "Work",           cx: 300.3, cy: 182.0 },
  { label: "Contact us",     cx: 364.2, cy: 182.0 },
  { label: "Life at exicon", cx: 422.7, cy: 208.0 },
  { label: "Careers",        cx: 465.5, cy: 255.5 },
];

/* Dots on TOP arc of Ring 1 (r=331.4) */
const BRANDS = [
  { name: "consulting",      src: "/consulting.svg",      dot: "#a7a9ac", cx: 229.9, cy: 17.3 },
  { name: "media-solutions", src: "/media-solutions.svg", dot: "#f58220", cx: 320.7, cy: 1.2 },
  { name: "contimedia",      src: "/contimedia.svg",      dot: "#f69220", cx: 412.4, cy: 10.9 },
  { name: "printle",         src: "/printle.svg",         dot: "#5e61a7", cx: 498.0, cy: 45.4 },
];

export default function NavOrbitMobile() {
  ...
  return (
    <div className="nav-orbit-mobile">
      <svg
        ref={svgRef}
        className="nav-orbit-mobile__svg"
        viewBox="0 0 664.53 664.87"
        overflow="visible"
        xmlns="http://www.w3.org/2000/svg"
      >
        {/* Orbit rings — same paths, just positioned at bottom */}
        ...
        {/* Nav items — TOP arc, text ABOVE dot */}
        {NAV_ITEMS.map((item) => (
          <g key={item.label} data-nav-item opacity="0" style={{ cursor: "pointer" }}
            onClick={() => handleNavClick(item.label)}>
            <circle cx={item.cx} cy={item.cy} r="4" fill="#c3884f" />
            <text x={item.cx} y={item.cy - 12} textAnchor="middle" fill="#a7a9ac"
              fontFamily="Bahnschrift, Segoe UI, sans-serif" fontWeight="300" fontSize="16">
              {item.label}
            </text>
          </g>
        ))}

        {/* Brand logos — TOP arc, logo BELOW dot (inside viewBox) */}
        {BRANDS.map((brand) => (
          <g key={brand.name} data-brand-item opacity="0">
            <circle cx={brand.cx} cy={brand.cy} r="5" fill={brand.dot} />
            <foreignObject x={brand.cx - 80} y={brand.cy + 10} width="160" height="40" overflow="visible">
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: "100%" }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={brand.src} alt={brand.name} style={{ height: "28px", width: "auto" }} draggable={false} />
              </div>
            </foreignObject>
          </g>
        ))}
      ...
      {/* Trigger — bottom center, half-circle points UP */}
      <button data-nav-trigger className="nav-orbit-mobile__trigger" onClick={toggleMenu}
        aria-label={menuOpen ? "Close navigation" : "Open navigation"}>
        <svg className="nav-orbit-mobile__trigger-svg" viewBox="0 0 55.25 130.51" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        <span className="nav-orbit-mobile__trigger-text">MENU</span>
      </button>
    </div>
  );
}
